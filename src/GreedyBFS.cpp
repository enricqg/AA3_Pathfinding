...
std::vector<Vector2D> GreedyBFS::CalculatePath(Node* initialPos, Node* targetPos)
{
	...
	while (!frontier.empty()) {
		...
		for (int i = 0; i < currentNeighbours.size(); i++) {
			...
			if (came_from.find(currentNeighbours[i]) == came_from.end()) {
				...
				int priority = (int)ManhattanDistanceHeuristic(targetPos->GetPosition(), currentNeighbours[i]->GetPosition()); //heuristic
...
Vector2D GreedyBFS::cell2pix(Vector2D cell)
{
    int offset = CELL_SIZE / 2;
    return Vector2D(cell.x * CELL_SIZE + offset, cell.y * CELL_SIZE + offset);
}

float GreedyBFS::ManhattanDistanceHeuristic(Vector2D n1, Vector2D n2)
{
